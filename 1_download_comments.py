...
#     for f in os.listdir(INPUT_DIR+"/"+current_profile):
#         # if f.endswith("UTC.json.xz"):
#         if "UTC.json.xz" in f:
#             try:
#                 media = json.loads(lzma.open(
#                     INPUT_DIR+"/"+current_profile+'/'+str(f), "r").read().decode("utf-8"))
...
#         print(now_str(), "Crawling post ", post_counter,
...
#             comments = get_comments_safe(iloader, code)
